...
\end{tabular}
一共用boot\_map\_region()初始化了三块区域，虚拟地址分别为UPAGES，KSTACKTOP - KSTKSIZE和KERNBASE。根据虚拟地址的前十位计算表项中的位置。其中KSTACKTOP - KSTKSIZE只是ULIM到KSTACKTOP中的一段区域，但是要求ULIM到KSTACKTOP - KSTKSIZE是无效的，相当于这一段也被映射了。
填出来的表如下：
...
\begin{figure}
  % Requires \usepackage{graphicx}
  \includegraphics[width=5in]{lab2ps.jpg}\\
\end{figure}
\begin{framed}
Challenge! Extend the JOS kernel monitor with commands to:
Display in a useful and easy-to-read format all of the physical page mappings (or
lack thereof) that apply to a particular range of virtual/linear addresses in the
currently active address space. For example, you might enter 'showmappings 0x3000
0x5000' to display the physical page mappings and corresponding permission bits that
apply to the pages at virtual addresses 0x3000, 0x4000, and 0x5000.
Explicitly set, clear, or change the permissions of any mapping in the current
address space.
Dump the contents of a range of memory given either a virtual or physical address
range. Be sure the dump code behaves correctly when the range extends across page
boundaries!
Do anything else that you think might be useful later for debugging the kernel.
(There's a good chance it will be!)
\end{framed}
这里主要是在monito.c中增加几条命令方便调试使用。
\begin{enumerate}
\item 显示虚拟页对应的物理页地址，实现很简单，只需要调用pgdir\_walk即可。然后显示pte的内容。具体的实现代码如下：
\begin{verbatim}
int
...
\item 设置也和1相同，需要的仅仅是改变*pte的控制位。主要的两个标志位U和W，代表用户权限，和写权限。具体实现代码如下：
\begin{verbatim}
int
mon_setmappings(int argc, char **argv, struct Trapframe *tf)
{
	if(argc!=3&&argc!=4)
		{
			cprintf("set, clear, or change the permissions 
...
\item 这里的难点是物理地址所代表内容的显示，由于*add这里的add是虚拟地址，所以需要把物理地址转换成虚拟地址。由于在pmap.c中，KERNELBASE的虚拟地址已经映射到0开始的物理地址上。
所以仅仅需要用KADDR将物理地址转换成虚拟地址即可，具体代码如下：
\begin{verbatim}
int
...
\begin{figure}
  % Requires \usepackage{graphicx}
  \includegraphics[width=5in]{lab2ex2_1.jpg}\\
\end{figure}
\begin{figure}
  % Requires \usepackage{graphicx}
  \includegraphics[width=5in]{lab2ex2_2.jpg}\\
\end{figure}
\end{enumerate}
\end{document} 